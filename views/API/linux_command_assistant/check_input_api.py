...
def analyze_input_with_gpt(input):
    role_description = "You are a system assistant for a Linux administrator. Your role is to answer user queries and alert the service if a query violates any policy. However, even if a query violates a policy, you must still provide an answer to the query."
    ...
    # 정책 위반 여부 확인
    if "violation" in gpt_response or "does not comply" in gpt_response:
